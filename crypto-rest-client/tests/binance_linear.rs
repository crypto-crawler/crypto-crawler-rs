...
mod linear_future {
    ...
    fn test_agg_trades() {
        let text =
            BinanceLinearRestClient::fetch_agg_trades("BTCUSDT_220624", None, None, None).unwrap();
        assert!(text.starts_with("[{"));
    }
    ...
    #[test]
    fn test_open_interest() {
        let text = fetch_open_interest("binance", MarketType::LinearFuture, Some("BTCUSDT_220624"))
            .unwrap();
        assert!(text.starts_with("{"));
    }
}
