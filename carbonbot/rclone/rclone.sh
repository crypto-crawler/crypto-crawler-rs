...
find $DATA_DIR -type f -name '*.json.gz' -mmin +3 -size -32c | xargs -r rm

# Delete json files older than 61 minutes and smaller than 64 bytes
find $DATA_DIR -type f -name '*.json' -mmin +61 -size -64c | xargs -r rm
