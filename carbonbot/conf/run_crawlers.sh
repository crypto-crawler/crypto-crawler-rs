...
REDIS_URL="redis://:password@ip:6379"   # optional
...
for msg_type in ${msg_types[@]}; do
  docker run -d --name carbonbot-$msg_type --restart always -v $LOCAL_DATA_DIR/data -e DATA_DIR=/data -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e AWS_S3_DIR=$AWS_S3_DIR -e REDIS_URL=$REDIS_URL -u "$(id -u):$(id -g)" soulmachine/carbonbot pm2-runtime start pm2.$msg_type.config.js
done
